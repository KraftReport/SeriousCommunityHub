...
let hasMore = true;
 ...
 function removePreviewForRawFile(){
    let parent = document.getElementById('editModal') 
    parent.innerHTML = '' 
 
}

 function removePreviewForPostUpdate(){
    let parent = document.getElementById('editModal') 
    parent.innerHTML = '' 
}
...
let currentPageForSavedPost = 0;
let isFetchingForSavedPost = false;
let hasMoreForSavedPost = true; 

let inPostTab = true
let inDelTab = false
let inSavedTab = false
...
const clickSavedPostTab = async () => {
    isFetchingForSavedPost = false
    hasMoreForSavedPost = true
    isFetchingForPost = false
    hasMoreForPost = true
    isFetchingForDelPost = false
    hasMoreForDelPost = true
    currentPageForSavedPost = 0
    inPostTab = false
    inDelTab = false
    inSavedTab = true
    document.getElementById('savedPostsDiv').innerHTML = ''
    await showSavedPosts()
}
...
window.addEventListener('scroll', async () => {
    ...
    if (isFetchingForSavedPost || !hasMoreForSavedPost) {
        console.log('here two')
        return;
    }
    ...
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight && inPostTab === false && inDelTab === false && inSavedTab ===true) {
        console.log('here')
        currentPageForSavedPost++;
        await videoObserver()
        await showSavedPosts();
        await videoObserver()
    }

    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight && inPostTab === true && inDelTab === false && inSavedTab === false) {
        console.log('here')
        currentPageForPost++;
        await videoObserver()
        await getPosts();
    ...
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight && inDelTab === true && inPostTab === false && inSavedTab === false) {
        console.log('here')
...
const showSavedPosts = async () => { 
      ...
      console.log(userId + 'wow wow wow')
       console.log('for currnet page',currentPageForSavedPost)
       let postsDiv = document.getElementById('savedPostsDiv')
       isFetchingForSavedPost = true
       let data = await fetch(`/post/getSavedPost/${currentPageForSavedPost}`)
       let response = await data.json()
       isFetchingForSavedPost = false
       console.log(response)
...
async function getAccessLogData(){
...
async function fetchUserDataByPostedUser(id){
 ...
 function extractMentionedUsersForComment  (postText){
...
async function deleteAllAccessLog(){
...
async function sendMentionNotification(mentionedUsers, id){
...
async function fetchPostedUser(id){
