...
document.body.addEventListener('hidden.bs.modal', async function (event) {
 ...
 async function getRejectedUser(id) {
     const data = await fetch(`/user/get-rejectedUser/${id}`);
     const res = await data.json();
     return res;
 }
...
async function populateCreateGroupFormForInvitation() {
    ...
    const rejectedUserIds = await getRejectedUser(communityId);
    const communityUsers = await getUsersByCommunityId(communityId);
    const usersNotInCommunity = await getUsersNotInCommunity(communityId);
    const usersList = document.getElementById('usersList');
    ...
    usersNotInCommunity.forEach(user => {
        ...
        if (rejectedUserIds.includes(user.id)) {
            const rejectedSpan = document.createElement('span');
            rejectedSpan.classList.add('rejected-user-forInvitation');
            rejectedSpan.style.marginTop = '-5px';
            rejectedSpan.style.padding = '4px';
            rejectedSpan.textContent = " (rejected)";
            nameSpan.appendChild(rejectedSpan);
        }
