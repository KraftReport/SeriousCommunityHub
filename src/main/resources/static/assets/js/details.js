...
function deleteResource(id) {
    ...
    // document.getElementById(id + '-url').classList.add('deleted-resource')
    document.getElementById(id + '-caption').value = ''
    console.log('removed')
    console.log(document.getElementById(id+'-url'))
...
document.addEventListener('DOMContentLoaded',async () => {
    let singlePost = localStorage.getItem('trendPostIdForSinglePost');
    await singlePagePost(singlePost);
     currentPage = '0';
     isFetching = false;
     hasMore = true;
...
const singlePagePost = async (id) => {
    ...
    let pWords = localStorage.getItem('commentOrReply');
    console.log('PWORDSDS',pWords)
    const showPost = document.getElementById('postDetailsForSingleForPost');
...
`
  ...
  <div class="post-content-${p.id} font-monospace" data-bs-toggle="modal" data-bs-target=${target} >
    ...
    if (pWords && (pWords.includes('comment') || pWords.includes('reply'))) {
        $('#commentStaticBox').modal('show'); // Ensure '#commentStaticBox' exists and is a valid modal element
        const commentElement = pressedComment(id);
        localStorage.removeItem('commentOrReply');
        if (commentElement && typeof commentElement.click === 'function') {
            await commentElement.click();
        } else {
            console.error('Error: pressedComment(id) did not return a valid clickable element.');
        }
    }
}
...
//         if (r.video === null) {
//             row += `
//         <img  style="width:100px; border-radius:10px; height:100px; height:100px;" alt="deleted"  id="${r.id}-url" value="${r.photo}" src ="${r.photo}" alt="Image not found" onerror="handleImageError(this)">
